  ...
  private handleRes = (res: AxiosResponse) => {
    ...
    if (responseCookie) {
      try {
        ...
        const parsedExistingCookie = existingCookie ? JSON.parse(existingCookie) : null;
        ...
        if (parsedExistingCookie) {
          ...
          const isEqualGoftinoStatus = parsedExistingCookie.goftinoStatus === parsedResponseCookie.goftinoStatus;
          const isEqualHighestRank = parsedExistingCookie.highestRank === parsedResponseCookie.highestRank;
          const isEqualroles = parsedExistingCookie.roles.length === parsedResponseCookie.roles.length;

          if (!isEqualCallStatus || !isEqualGoftinoStatus || !isEqualHighestRank || !isEqualroles) {
