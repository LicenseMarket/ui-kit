...
const Button = memo(
  ({ emoji, index, onMouseOver, onSelect }: EmojiButtonProps) => {
    return (
      <button
        className="group relative flex size-9 cursor-pointer items-center justify-center
          border-none bg-transparent text-2xl leading-none"
    ...
    return emojiLibrary
      ...
      .map(({ id: categoryId }) => {
        ...
        return (
          ...
          >
            <div
              className="bg-popover/90 sticky -top-px z-[1] p-1 py-2 text-sm font-semibold
                backdrop-blur-sm"
            >
  ...
  const SearchList = useCallback(() => {
    return (
      <div style={{ width: getRowWidth }} data-id="search">
        <div
          className="bg-popover/90 text-card-foreground sticky -top-px z-[1] p-1 py-2 text-sm
